...
class Employee {
    ...
    getEmployees() {
     return db.query('SELECT e.first_name, e.last_name, r.title, d.name as department, r.salary, e.manager_id FROM employee AS e JOIN role AS r on r.id = e.role_id INNER JOIN department AS d on d.id = r.department_id;',
      )
    }
    addEmployee(id, first_name, last_name, role_id, manager_id) {
        ...
        // add to department
        // add to role
    return db.query("INSERT INTO employee(first_name, last_name, role_id, manager_id) VALUES(?)", (id, first_name, last_name, role_id, manager_id), (err, results) => {
            if (err) {
                console.log(err);
              } else {
